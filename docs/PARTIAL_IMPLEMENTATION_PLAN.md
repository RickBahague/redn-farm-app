# Plan — Partial Story Closure Record

**Canon:** [`USER_STORIES.md`](./USER_STORIES.md) (status lines are authoritative).  
**Created:** 2026-04-09  
**Purpose:** Closure record for previously partial stories and a handoff pointer for remaining ongoing documentation sync.

---

## How to use this document

1. Use the **Closure summary** section to confirm what was completed and verified.
2. Treat **Stream H — Documentation consistency** as the only active continuation track in this file.
3. Use stream sections below as historical implementation records unless explicitly reopened.

---

## Closure summary (2026-04-09)

This section captures closure status after the final verification pass and cross-doc sync.
...
The items below were the former partials and are now closed.
...
Only **Stream H — Documentation consistency (ongoing)** is active from this file.

- Keep `USER_STORIES.md`, `EOD_EPIC_TRACKER.md`, and relevant epic trackers in sync whenever new delivery slices land.
- Re-verify `DESIGN.md` and `schema_evolution.sql` when navigation/database changes are introduced.
- If new partial stories appear, create a new execution section rather than reusing this closed checklist.
...
> Historical implementation record (archived). Do not treat this section as an active todo list unless explicitly reopened.
...
| Doc | Action |
|-----|--------|
| [`EOD_EPIC_TRACKER.md`](./EOD_EPIC_TRACKER.md) | Synced during closure pass; keep top **Implementation snapshot** current when future slices land. *(verified 2026-04-09)* |
| [`PHASE6_TRACKER.md`](./PHASE6_TRACKER.md) | DB version **10**; P6-4 synced (Stream D). Re-check on next DB bump. *(verified 2026-04-09)* |
| [`DESIGN.md`](./DESIGN.md) | Navigation table + `FarmDatabase` version checked against `NavGraph.kt` / `FarmDatabase.kt`. *(verified 2026-04-09)* |
| [`schema_evolution.sql`](./schema_evolution.sql) | Header + current VERSION reference aligned to DB **v10**; re-verify when bumping Room. *(verified 2026-04-09)* |

### Stream H verification snapshot (2026-04-09)

- `FarmDatabase.kt` remains `@Database(version = 10)`.
- `schema_evolution.sql` header and latest-version reference remain aligned to v10.
- `DESIGN.md` navigation table and DB version note checked against `NavGraph.kt` and `FarmDatabase.kt`.
- `EOD_EPIC_TRACKER.md` closure slice + implementation snapshot are synchronized with `USER_STORIES.md` statuses.
- `PHASE6_TRACKER.md` naming note refreshed to reflect shipped `EXP-US-02` status.
- `DESIGN.md` Epic 12 RBAC wording aligned to current `Rbac.kt` for Day Close History and Outstanding Inventory access.

**Stream H closure seal (2026-04-09):** No known cross-doc drift remains for the verified scope (DB versioning, navigation/RBAC wording, Epic 12 closure statuses, and tracker sync). Re-open Stream H checks when new feature slices land.
...
|-------|------------------|------------------------------|
| `EOD-US-04` | ✅ shipped (cash discrepancy/remarks guard + copy/test alignment) | `EOD_EPIC_TRACKER.md`, `USER_STORIES.md` |
| `EOD-US-09` | ✅ shipped (employee summary/notes/thermal wording parity) | `EOD_EPIC_TRACKER.md`, `USER_STORIES.md`, `DESIGN.md` |
| `EOD-US-10` | ✅ shipped (outstanding inventory AC3 ledger fields + unit labeling) | `EOD_EPIC_TRACKER.md`, `USER_STORIES.md`, `printing.md` |
