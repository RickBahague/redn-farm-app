...
**Historical checklist note:** Phase sections below are retained as implementation history and may still show pre-closure `[ ]` entries from the original build plan. For current acceptance state, use **Implementation snapshot** + **Closure slice for remaining partials** at the top and `USER_STORIES.md` status lines as canon.
