...
The product is an Android app, which will also have a companion web app based on Drupal in a separate product.
...
Users are created in `FarmDatabase`'s `onOpen` / `onCreate` callback if they don't already exist. Role is stored as a plain string in `users.role`. Dashboard and several flows already gate destinations using **`security/Rbac.kt`** and **`SessionManager`** (normalized role strings **`ADMIN`**, **`STORE_ASSISTANT`**, **`PURCHASING`**, etc.). **Epic 12 (EOD)** adds further gates per current `Rbac` policy (e.g. Close Day for admin/store assistant/purchasing; Outstanding Inventory for admin/purchasing/store assistant; Day Close History for admin/store assistant/purchasing; un-finalize remains admin-only).
...
- **Dashboard — Day Close History** (`EOD-US-06`): **`ADMIN`**, **`STORE_ASSISTANT`**, **`PURCHASING`**.
- **Dashboard — Outstanding Inventory** (`EOD-US-10`): **`ADMIN`**, **`PURCHASING`**, **`STORE_ASSISTANT`**.
